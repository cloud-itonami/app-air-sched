(ns air-sched.route
  ...
  build, or a network. `air-sched.worker` is the only namespace that touches
...
(def routes
  ...
  docs/adr/0001 records was a page carrying `routeCount` as a literal beside a
  `wrangler.jsonc` that declares the real number."
...
(defn- xrpc-nsid
  ...
  （`+server.ts`: `const nsid = event.params.path; if (!nsid) …400`）。
