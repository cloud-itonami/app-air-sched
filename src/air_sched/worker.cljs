(ns air-sched.worker
  ...
  ここには判断を置かない —— どのハンドラが答えるかは `air-sched.route/dispatch`
  が決め、ページの中身は `air-sched.view` が組む。どちらも `.cljc` なので、
  ブラウザもビルドも無しにテストできる。
  ...
  (:require [air-sched.route :as route]
...
(defn- env->map
  ...
  （中継先の URL だけは route/mcp-router-url を通して意図的に表示する）。"
  [env]
  (if env
...
(defn fetch-handler [req env _ctx]
  (let [url (js/URL. (.-url req))
    ...
    (case action
      ...
      :health (json {:ok true :app "air-sched" :runtime "cljs"
