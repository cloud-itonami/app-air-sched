...
;; (test/air_sched/route_test.cljc) はソースの判断を固定するが、bundle が
...
(def sentinel
  ...
  つまり検査が構造的に落ちなくなる。app-ongakuka の実測でこれを踏んだので印を使う。"
  "SENTINEL-7c41d9")
...
(def env #js {"APP_NANOID" "a1rsch3d"
              "APP_UI_TYPE" sentinel
              "AGENTGATEWAY_MCP_ROUTER_URL" router-url})

(defn- call [h method path]
  (let [req (js/Request. (str "https://air-sched.etzhayyim.com" path) #js {:method method})]
         ...
         (-> (js/Promise.all
             ...
             (.then
              (fn [[page health bad pre nf mna]]
                ...
                ;; （app-ongakuka 2026-08-18 の実測: sentinel が表示されない var に
                ;; 付いていたため、実際に出ている唯一の値を検査できていなかった）。
                ...
                ;; DDS の検査は **2 つに割る。** 「dads-table が在る」だけでは
                ;; 落ちない —— それは view が出力する markup であって、CSS が
                ;; 1 バイトも入っていないページにも現れる。実測 2026-08-18
                ;; （このページ）: `dads-table` は css 込み 74 / css 無し **6**
                ...
                ;; 多段（/xrpc/a/b）は移行前の [...path] と同じく転送するので
                ;; ここでは叩かない —— 実 fetch になり、mcp.etzhayyim.com が
                ;; いま NXDOMAIN であることに寄りかかる検査になるため。
                ;; 判定は unit test（route_test）の方に置いてある。
