...
(deftest dispatch-xrpc
  (testing "単一セグメントの nsid"
    (is (= {:action :xrpc :nsid "com.etzhayyim.apps.airSched.registerSchedule"}
           (route/dispatch "POST" "/xrpc/com.etzhayyim.apps.airSched.registerSchedule"))))
...
(deftest page-shows-the-real-routes
  (testing "ページは route 表から描く。固定値を焼かない（docs/adr/0001 の欠陥）"
    (let [html (view/render {:css "/*x*/" :routes route/routes
      ...
      (testing "route 表を空で渡せば route 行は 1 本も出ない（焼いていない証拠）"
        (let [empty-html (view/render {:css "/*x*/" :routes [] :vars []
                                       :mcp-url "https://mcp.example/x"})]
          (is (not (str/includes? empty-html "/xrpc/:nsid"))))))))
